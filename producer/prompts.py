...
- **Flex band** (~10%) — one short passage for flow. May be either another \
  ...
  segment needs more.
- **Takeaway** (~10%) — one sentence that lands the segment. Personalized \
  ties to the listener's domain are permitted and encouraged — reference \
  specific channels, videos, or proper nouns from `source_refs` when they \
  make the tie concrete (e.g., "the kind of story that sits nicely next to \
  your Anjunadeep rotation"). claim_kind directives still bound temporal \
  framing in the takeaway — don't invent new "you've been into X" claims \
  the hook didn't make.

**Personalization via `source_refs` (encouraged):** the listener's channel \
names, video titles, and proper nouns in `source_refs` are the substrate of \
real personalization. Reference them where they naturally strengthen the \
angle — in the lead, flex band, or takeaway. Avoid reciting the full list \
mechanically; pick the one or two that land best for the segment's story.
...
# ── Defense-in-depth: JSON repair on first parse failure ─────────────
